...
pico = serial.Serial('COM7', baudrate=115200)
...
def doe1_Latchup(expectedLV, expectedHV, latchup_recovery_time):
    ...
    try:
        while statement is True:
            ...
            if (len(current_list) > repreated_above_threshold) and (all_measure_LV or all_measure_HV):
                print("Latchup")
                smu._write(value='smua.source.output = smua.OUTPUT_OFF')
                smu._write(value='smub.source.output = smub.OUTPUT_OFF')
    ...
    print(f"Latchups: {latchup_count}")
...
if test == 1:
    ...
    doe1_Latchup(expectedLV=.00016, # LV threshold
                    expectedHV=.010, # HV threshold
                    latchup_recovery_time=2
    )
elif test == 3:
    print("Op amp Transient test is selected.")
    doe1_Tranisents(amp=0, voltage=.7, seconds=30) # desired voltage
elif test == 4:
    ...
    if path.exists():
        ...
        seuData = pd.DataFrame(data=[], index=[], columns=["Beam Energy", "Ion", "LET", "Fluence", "Ion Energy", "Flux", "Time",
                                                            "HorizontalSR_Upsets", "VerticalSR_Upsets", 
                                                            "Test", "H_DataState", "V_DataState",
                                                            "H_XS", "V_XS"])
        testNumber = 0
    ...
    test_time = 100
    flux = 100000
    h_dataS = str("h") #str(input("Horizontal steady state condition: "))
    v_dataS = str("h") # str(input("Vertical steady state condition: "))
    ...
    print(braggCurve)
    let = braggCurve.LET.loc[braggCurve.depth ==  1.07]
    energy = braggCurve.MEV.loc[braggCurve.depth ==  1.07]
    # print(energy.loc[1])
    ...
    seuData.at[testNumber, "Beam Energy"] = 16
    seuData.at[testNumber, "Ion"] = "Oxygen"

...
for c in range(rowNum):
    for c in range(32, colNum):
        ...
        rowRX = pico.read_until().stri
        p().decode()                                   # confirms column selected
        ...
        if commandRX == 1:
            # for i in range(1):
                ...
                # smu.smub.measure.v()
                # time.sleep(.5)
            vOut = smu.sourceA_Timed(smu.smua, smu.smub, 0.000000005, 20, .001, .0005)
            # smu.apply_current(smu.smua, 0.000000005)
                # bData[rowS] = vOut
